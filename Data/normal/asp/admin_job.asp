...
<table width="100%" border="0" cellspacing="0" cellpadding="0"  class="list_table">
  <tr> 
    <th height="34" align="center"><strong>人才招聘</strong></th>
  </tr>
  <tr> 
  </tr>
</table>
...
%>
    <tr  bgcolor="#ffffff" onMouseOver=javascript:this.bgColor='F7FCFF' onMouseOut=javascript:this.bgColor='#ffffff'>
      ...
      <td><%=rs("id")%></td>
      <td><%=left(rs("jobname"),40)%></td>
      <td><div align="center"><%=rs("renshu")%></div></td>
      <td><div align="center"><%=rs("nianlinga")%>-<%=rs("nianlingb")%></div></td>
      <td><div align="center"><%=rs("xingbie")%></div></td>
      <td><div align="center"><%=rs("sj")%></div></td>
      <td><div align="center"><%=rs("daiyu")%></div></td>
	  <td><div align="center"><a href="admin_job_edit.asp?id=<%=rs("id")%>">修改</a></div></td>
...
rs.delete
rs.update
Response.Write "<script>alert('当前信息删除成功！');window.location.href='admin_job.asp';</script>"
end if 

if Request.QueryString("action")="del" then
Response.Write "<script>alert('恭喜!删除成功!');window.location.href='?';</script>" 
end if
